...
async def register(
        data: UserRegister,
        usecase: RegisterUser = Depends(register_user_service),
):
...
async def login(
        ...
        usecase: LoginUser = Depends(login_user_service),
):
    try:
...
async def request_recovery(
        email: EmailStr,
        generate_uc: GenerateRecoveryCode = Depends(generate_recovery_code_service),
        send_uc: SendRecoveryCode = Depends(send_recovery_code_service),
):
...
async def verify_code(
        usuario_id: str,
        code: str,
        usecase_code: VerifyRecoveryCode = Depends(verify_recovery_code_service),
        usecase_auth: VerifyUser = Depends(verify_user_service)
):
...
async def reset_password(
        usuario_id: str,
        new_password: str,
        usecase: ResetPassword = Depends(reset_password_service),
):
...
async def submit_consentimiento(
    ...
    user: AuthenticatedUser = Depends(get_current_user),
    usecase: ConfirmConsent = Depends(confirm_consent_service)
):
    try:
        usecase.execute(

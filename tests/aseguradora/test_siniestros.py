"""Pruebas de §3 Aseguradora (`/api/v1/aseguradora/...`) — siniestros + acciones de flujo."""
import pytest
...
def wired(client):
    ...
    app.dependency_overrides[sdeps.editar_siniestro_service] = lambda: EditarSiniestro(sin)
...
def test_editar_siniestro(wired):
    r = wired["client"].put(f"{BASE}/siniestros/sin-rep", json={"narracion_texto": "Actualizado"})
    assert r.status_code == 200, r.text
    assert r.json()["narracion_texto"] == "Actualizado"

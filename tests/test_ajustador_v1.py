...
from tests.fakes import (
    FakeAjustadorRepo, FakeSiniestroRepo, FakeImagenRepo, FakePeritajeRepo,
    default_ajustador, make_siniestro,
)
...
def test_asignaciones_paginado(wired):
    ...
    # 2 siniestros pertenecen a aj-profile-1 (sin-1 y sin-prelim)
    assert body["total"] == 2
...
def test_registrar_peritaje_estatus_invalido_409(wired):
    ...
    assert r.status_code == 409  # transición Reportado_Preliminar → Peritaje_Validado inválida
...
def test_editar_tras_validar_bloqueado(wired):
    # Primero validamos el siniestro vía POST, luego intentamos editar
